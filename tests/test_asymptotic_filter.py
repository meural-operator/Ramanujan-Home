import unittest
from ramanujan.utils.asymptotic_filter import is_asymptotically_convergent
...
class TestAsymptoticFilter(unittest.TestCase):
    ...
    def test_worpitzky_boundary(self):
        # a_deg * 2 == b_deg
        # 4 * b_n < -a_n^2 -> converges
        self.assertTrue(is_asymptotically_convergent(a_deg=2, a_leading_coef=2, b_deg=4, b_leading_coef=-2))
        
        # 4 * b_n >= -a_n^2 -> divergent
        self.assertFalse(is_asymptotically_convergent(a_deg=2, a_leading_coef=2, b_deg=4, b_leading_coef=-1))
